...
{
	JPanel stateSliderPanel; //JKevin
	...
	/** Delay timer to allow GUIParent to initialize (to avoid null point exceptions) // added by Justin */
	...
	public StatePanel(ControlPanel cPanel, boolean initialPause)
	{
		...
		// Below commented out, because the top GuiParent is not initialized yet, hence the NPE. Added by Justin
		/*try
		{
		...
		delayTimer.schedule(new TimerTask() {
		    ...
		    4000);//how long to wait before running task
		...
		stateSliderPanel = new JPanel(); //JKEvin
		...
		// As above (line 106), this is commented out and completed in doDelayedTasks(), added by Justin
		/*try
		{
		...
		//this.add(stateButtonPanel); //JKevin
		this.add(stateSliderPanel);
		//this.add(colorLinesPanel); //JKevin
	...
	public JPanel getStateSliderPanel() {
		return stateSliderPanel;
	}
}
